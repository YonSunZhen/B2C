...
namespace GoosleB2C.BLL
{
	...
	{
		private readonly GoosleB2C.DAL.IndexImg dal=new GoosleB2C.DAL.IndexImg();
		public IndexImg()
		{}
		#region  Method
		/// <summary>
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public GoosleB2C.Model.IndexImg GetModelByCache(string id)
		{
			
			string CacheKey = "IndexImgModel-" + id;
			...
			return (GoosleB2C.Model.IndexImg)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<GoosleB2C.Model.IndexImg> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<GoosleB2C.Model.IndexImg> DataTableToList(DataTable dt)
		{
			List<GoosleB2C.Model.IndexImg> modelList = new List<GoosleB2C.Model.IndexImg>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				GoosleB2C.Model.IndexImg model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new GoosleB2C.Model.IndexImg();
					...
					if(dt.Rows[n]["updatePeople"]!=null && dt.Rows[n]["updatePeople"].ToString()!="")
					{
					model.updatePeople=dt.Rows[n]["updatePeople"].ToString();
					}
					modelList.Add(model);
				}
			}
